...
class Ghost:
    def __init__(self, plane_size=100):
        # CORREGIDO: Añadida la ruta 'visuales/'
        ...
        self.angle_y = 0.0      # <-- AÑADIDO
        self.bound_margin = 8.0 # <-- AÑADIDO (y activado)
    ...
    # CORREGIDO: Eliminados 'grid_x' y 'grid_y' de los argumentos
    def update(self, dt):
        """Actualiza posición con movimiento aleatorio"""
        # ELIMINADO:
...
class Personaje:
    def __init__(self, model_paths):
        ...
        # CORREGIDO: Usando las llaves correctas del diccionario
...
def Init():
    ...
    # CORREGIDO: Rutas apuntando a la carpeta 'visuales/'
    model_paths = {
    ...
    # CORREGIDO: Añadida la ruta 'visuales/'
    keys_list = [
        Key('visuales/key.obj', pos=[10, 5, 30], scale=4.0),
        Key('visuales/key.obj', pos=[-30, 5, -20], scale=4.0),
        Key('visuales/key.obj', pos=[20, 5, -20], scale=4.0)
    ]
...
def display(dt, is_moving):
    ...
    # ELIMINADO: Bloque 'try...except' que conectaba con Julia
    
    personaje.update(dt, is_moving)
    # CORREGIDO: Se llama a ghost.update solo con 'dt'
    ghost.update(dt)
...
while not done:
    ...
    pygame.display.flip()
    # pygame.time.wait(100) # Puedes descomentar esto si corre muy rápido
